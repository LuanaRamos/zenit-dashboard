...
def render_profile_campaigns(campaigns: list[CampaignInsight]) -> None:
    ...
    data = [{"Campanha": c.campaign_name, "Gastos": round(c.spend, 2), "Cliques no Criativo": c.other_clicks, "Cliques no Link": c.link_clicks, "CPC": round(c.cpc, 2), "Visitas": c.profile_visits, "Seguidores": c.instagram_follows, "Custo/Seg": round(c.cost_per_follower, 2)} for c in campaigns]
    render_glass_table(
        pd.DataFrame(data),
...
def render_general_campaigns(campaigns: list[CampaignInsight], title: str = "Outras Campanhas") -> None:
    if not campaigns: return
    st.markdown(f"### {title}")
    data = [{"Campanha": c.campaign_name, "Objetivo": c.objective_friendly, "Gastos": round(c.spend, 2), "Impr": c.impressions, "Cliques no Criativo": c.other_clicks, "Cliques no Link": c.link_clicks, "Leads (Site)": c.site_leads, "Leads (Form)": c.native_leads, "CPL": round(c.cpl, 2), "CPM": round(c.cpm, 2)} for c in campaigns]
