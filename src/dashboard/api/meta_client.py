...
class MetaAdsClient:
    ...
    def get_ads_reach_mapping(self) -> dict[str, Any]:  # noqa: C901
        """
        ...
        try:
            insights_data = self._make_request(insights_endpoint, insights_params).get(
                "data", []
            )
        ...
        try:
            ...
            if "created_time" in data:
                return datetime.datetime.strptime(
                    data["created_time"][:10], "%Y-%m-%d"
                ).date()
        except Exception as e:
            logger.warning(
                f"Erro ao buscar created_time da ad account, usando fallback: {e}"
            )
